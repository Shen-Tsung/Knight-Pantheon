...
function TextAnim.new(text, delay, x, y, soundPath)
    ...
    if soundPath then
        self.sound = love.audio.newSource(soundPath, "static")
    else
        self.sound = nil
    end
...
function TextAnim:setSound(soundPath)
    if soundPath then
        self.sound = love.audio.newSource(soundPath, "static")
    else
        self.sound = nil
...
function TextAnim:update(dt)
    if not self.isComplete and self.currentChar < #self.chars then
        ...
        if self.time >= self.delay then
            ...
            if self.sound then
				self.sound:setVolume(saveData["options"]["sound"])
                self.sound:seek(0)
                self.sound:play()
            end
